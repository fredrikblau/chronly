...
the browser's extension storage. Larger operational data uses local storage;
small settings and world-clock preferences may use browser sync storage so the
browser can sync them across signed-in profiles. Chronly does not operate that
sync service and websites cannot read the extension storage.
